...
public class GifSequenceWriter {
	...
	public GifSequenceWriter(ImageOutputStream outputStream, int imageType,
		...
		updateMetaDataNode(timeBetweenFramesMS, loopContinuously);
	...
	private void updateMetaDataNode(int timeBetweenFramesMS,
	...
	public void writeToSequence(RenderedImage img, int millis) throws IOException {
		updateMetaDataNode(millis, false);
	...
	public static void main(String[] args) throws Exception {
		if (args.length > 1) {
			...
			writer.writeToSequence(firstImage, 1000);
			for (int i = 1; i < args.length - 1; i++) {
				BufferedImage nextImage = ImageIO.read(new File(args[i]));
				writer.writeToSequence(nextImage, 1000);
			}
