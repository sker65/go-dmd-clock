...
public class DMD extends Observable {
	...
	// the original frame that holds recording data
	private Frame oFrame = new Frame();
	// the frame that holds background data
	...
	public void copy(int yoffset, int xoffset, DMD src, boolean low, boolean mask) {
		...
		for (int row = 0; row < src.getHeight(); row++) {
			for (int col = 0; col < src.getWidth() / 8; col++) {
				if (mask)
					frame1[(row + yoffset) * getBytesPerRow() + xoffset
							+ col] &= ~src.frame.getPlane(0)[src.getBytesPerRow() * row + col];
				else
					frame1[(row + yoffset) * getBytesPerRow() + xoffset + col] = src.frame
							.getPlane(0)[src.getBytesPerRow() * row + col];

				if (!low) {
					if (mask)
						frame2[(row + yoffset) * getBytesPerRow() + xoffset
								+ col] &= ~src.frame.getPlane(0)[src.getBytesPerRow() * row + col];
					else
						frame2[(row + yoffset) * getBytesPerRow() + xoffset + col] = src.frame
								.getPlane(0)[src.getBytesPerRow() * row + col];
	...
	private int[] pixmask = { 0b01111111, 0b11011111, 0b11110111, 0b11111101, 0b10111111, 0b11101111, 0b11111011,
			0b11111110 };
	...
	public void setOriginalFrame(Frame oFrame) {
		log.trace("setOriginalFrame(Frame@{})");
		this.oFrame = oFrame;
	}

	public Frame getOriginalFrame() {
		log.trace("getOriginalFrame()");
		return oFrame;
	}

	public void setBackground(Frame bgFrame) {
		log.trace("setBackground(Frame@{})");
		this.bgFrame = bgFrame;
	}

	public Frame getBackground() {
		log.trace("getBackground()");
		return bgFrame;
	}
	...
	public void dumpHistogram() {
		...
		log.trace("DMD@{} with Frame@{} hist: {}, {}", Integer.toHexString(hashCode()),
				Integer.toHexString(frame.hashCode()), this.dumpHist(), this.toString());
	}
	...
	/**
	 * binary mask for drawing planes. bit 0 -> mask plane bit 1 -> first color
	 * plane ....
	 * 
	 * @param drawMask
	 */
	...
	public String toString() {
		return String.format(
				"DMD [width=%s, height=%s, bytesPerRow=%s, planeSize=%s, drawMask=%s, frame=%s, actualBuffer=%s, mask=%s]",
				width, height, bytesPerRow, planeSize, drawMask, frame, actualBuffer, Arrays.toString(pixmask));
	}
